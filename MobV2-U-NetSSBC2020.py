...
PATH = "/Users/omkar/Desktop/MU-Net/data"

X_train = np.zeros((Number_of_traininSet, IMAGE_SIZE, IMAGE_SIZE,1), dtype=np.uint8)
Y_train = np.zeros((Number_of_traininSet, IMAGE_SIZE, IMAGE_SIZE,1), dtype=np.bool_)

def normalize_filename(name):
    return name.replace(" ", "").replace(".jpg", "").replace(".png", "").lower()
...
def load_data(path, split=0.1):
    ...
    for sub in glob(os.path.join(image_dir, "*")):
        for f in glob(os.path.join(sub, "*.jpg")):
            key = normalize_filename(os.path.basename(f))
            image_paths[key] = f

    # Collect all mask paths
    for sub in glob(os.path.join(label_dir, "*")):
        for f in glob(os.path.join(sub, "*.png")):
            key = normalize_filename(os.path.basename(f))
            label_paths[key] = f

    # Only keep entries where both image and mask exist
    common_keys = sorted(set(image_paths.keys()) & set(label_paths.keys()))
    images = [image_paths[k] for k in common_keys]
    masks = [label_paths[k] for k in common_keys]

    print(f"Found {len(images)} image–mask pairs.")

    total_size = len(images)
    valid_size = max(1, int(split * total_size))
    test_size = max(1, int(split * total_size))

    train_x, valid_x, train_y, valid_y = train_test_split(images, masks, test_size=valid_size, random_state=42)
